...
def train(question, augments, args, model, tokenizer, 
    ...
    if args.freeze_A:
        # print(f"Total number of trainable parameters before freezing: {sum([p.numel() for p in model.parameters() if p.requires_grad])}")
    ...
    # print(f"Total number of trainable parameters: {sum([p.numel() for p in model_parameters])}")
...
def main(args):
    ...
    if not args.use_warmup:
        ...
        init_adapter_path = os.path.join(
            ...
            args.model_name,
            f"rank{args.lora_rank}_alpha{args.lora_alpha}",
            "cot" if args.with_cot_warmup else "direct", 
            f"epochs{args.warmup_epoch}" # Warmup epoch >< args.num_train_epochs
        )
        assert os.path.exists(os.path.join(init_adapter_path, "adapter_model.safetensors")), f"No warmup LoRA weight in {init_adapter_path}, must run warmup first."
    ...
    for filename, fulldata in data_list:
        ...
        output_dir = os.path.join(
            ...
            f"warmup_epoch{args.warmup_epoch}" if args.warmup_epoch > 1 else "", # only need new folder when warmup_epoch > 1, otherwise look at its parent folder
            args.model_name, 
            f"rank={args.lora_rank}_alpha={args.lora_alpha}",
            args.dataset,
...
if __name__ == "__main__":
    ...
    # Warm up + Hydra architecture
    parser.add_argument("--use_warmup", action="store_true")
    parser.add_argument("--freeze_A", action="store_true")
    parser.add_argument("--warmup_epoch", type=int, default=1, help="Number of epochs that warmup LoRA was trained for")
    parser.add_argument("--with_cot_warmup", action="store_true", help="Whether the warmup LoRA was trained with CoT")
    args = parser.parse_args()
    assert args.lora_rank and args.lora_alpha, "No config for LoRA"
    if args.augment_model is None:
        args.augment_model = args.model_name
    if (args.freeze_A or args.with_cot_warmup) and not args.use_warmup:
        raise ValueError("Any warmup setting can only be used when use_warmup is set.")
    print(args)
    main(args)